...
from ripper.ripperlib.defs import SpreadsheetProperties, SheetsService
...
class TestSheetDataCache(unittest.TestCase):
    ...
    def test_get_sheet_data_exact_cache_hit(self) -> None:
        ...
        self.assertEqual(load_source, "cache")
    ...
    def test_get_sheet_data_cache_miss(self) -> None:
        ...
        with patch("ripper.ripperlib.sheet_data_cache.fetch_data_from_spreadsheet") as mock_fetch:
            ...
            self.assertEqual(load_source, "api")

            # Verify API was called with correct parameters
            mock_fetch.assert_called_once_with(
                self.mock_sheets_service, self.test_spreadsheet_id, f"{self.test_sheet_name}!{range_a1}"
            )
    ...
    def test_get_sheet_data_partial_overlap_combined(self) -> None:
        ...
        # Setup mock API response for missing part (A1:C3, but API returns full range)
        api_data = [["A1", "B1", "C1"], ["A2", "B2", "C2"], ["A3", "B3", "C3"]]
        ...
        with patch("ripper.ripperlib.sheet_data_cache.fetch_data_from_spreadsheet") as mock_fetch:
            ...
            self.assertEqual(result_data, api_data)
            self.assertEqual(load_source, "api")  # Fallback to API when overlap detected
            ...
            # Verify new data was cached (replacing old range)
            cached_data_after = self.db.get_sheet_data_from_cache(
                self.test_spreadsheet_id, self.test_sheet_name, 1, 1, 3, 3
            )
            self.assertEqual(cached_data_after, api_data)
    ...
    def test_get_sheet_data_multiple_ranges_combined(self) -> None:
        ...
        # Setup mock API response for the full range
        api_data = [
            ["A1", "B1", "C1", "D1", "E1"],
            ["A2", "B2", "C2", "D2", "E2"],
            ["A3", "B3", "C3", "D3", "E3"],
        ...
        with patch("ripper.ripperlib.sheet_data_cache.fetch_data_from_spreadsheet") as mock_fetch:
            ...
            self.assertEqual(result_data, api_data)
            self.assertEqual(load_source, "api")  # Falls back to API due to gaps

            # Verify API was called
            mock_fetch.assert_called_once()

    def test_get_sheet_data_invalid_range(self) -> None:
        """Test handling of invalid A1 notation."""
    ...
    def test_logging_behavior(self, mock_logger: MagicMock) -> None:
        ...
        cache_hit_logged = any("Returning data from cache" in msg for msg in debug_calls)
